...
void translate_program(ASTNode* root){
    ...
    codes->tail = NULL;
    if (root->child[0]) {
        translate_extdeflist(root->child[0], codes);
...
void translate_extdeflist(ASTNode* root, intercodes* codes){
    if(!root)
        return;
    translate_extdef(root->child[0],codes);
    intercodes *codes1 = malloc(sizeof(intercodes));
    codes1->head = NULL;
    codes1->tail = NULL;
    translate_extdeflist(root->child[1],codes1);
...
void translate_extdef(ASTNode* root, intercodes* codes){
    if(root->num==3&&strcmp(root->child[2]->name,"CompSt")==0){
        ...
        translate_compst(root->child[2],codes1);
...
void translate_compst(ASTNode* root, intercodes* codes){
    translate_deflist(root->child[1],codes);
    translate_stmtlist(root->child[2],codes);
}
void translate_stmtlist(ASTNode* root, intercodes* codes){
    if(!root){
        return;
    }
    intercodes* codes1=translate_stmt(root->child[0]);
    intercodes_merge(codes,codes1);
    translate_stmtlist(root->child[1],codes);
...
void translate_deflist(ASTNode* root, intercodes* codes){
    ...
    translate_def(root->child[0],codes);
    translate_deflist(root->child[1],codes);
}
...
intercodes* translate_exp(ASTNode* root,operand* op){
    ...
    if(root->num==1){
        ...
        if(strcmp(root->child[0]->name,"MINUS")==0){
            ...
            intercodes* codes1=translate_exp(root->child[1],op1);
            intercodes_merge(codes,codes1);
        ...
        if(strcmp(root->child[1]->name,"ASSIGNOP")==0){
            ...
            if(root->child[0]->num==1){
                ...
                if(exp_int(root->child[2])==1){                  
                    ...
                    intercodes* codes1=translate_exp(root->child[2],op1);
                    intercodes_merge(codes,codes1);
            ...
            if(exp_int(root->child[0])==0&&exp_id(root->child[0])==0){
                intercodes* codes1=translate_exp(root->child[0],op1);
                intercodes_merge(codes,codes1);
            }

            if(exp_int(root->child[2])==0&&exp_id(root->child[2])==0){
                intercodes* codes2=translate_exp(root->child[2],op2);
        ...
        if(strcmp(root->child[1]->name,"LP")==0){
            ...
            intercodes* codes1=translate_args(root->child[2],&args);
            intercodes_merge(codes,codes1);
...
intercodes* translate_stmt(ASTNode* root){
    ...
    if(root->num==1){
        ...
        if(exp_int(root->child[1])==0&&exp_id(root->child[1])==0){
            intercodes* code1=translate_exp(root->child[1],op1);
            intercodes_merge(codes,code1);
        }
        ...
        if(strcmp(root->child[0]->name,"IF")==0){
            ...
            intercodes* codes1=translate_cond(root->child[2],label1,label2);
            intercodes_merge(codes,codes1);
            ...
            intercodes* codes2=translate_stmt(root->child[4]);
            intercodes_merge(codes,codes2);
            ...
            intercodes* codes1=translate_cond(root->child[2],label2,label3);
            intercodes_merge(codes,codes1);
        ...
        intercodes* codes3=translate_stmt(root->child[6]);
...
intercodes* translate_cond(ASTNode* root,int label1,int label2){
    ...
    if(root->num==2&&strcmp(root->child[0]->name,"NOT")==0){
        ...
        intercodes* codes2=translate_cond(root->child[2],label1,label2);
        intercodes_merge(codes,codes2);
...
intercodes* translate_args(ASTNode* root,arglist** arg_head){
    ...
    if(root->num==1){
        ...
        intercodes* code1=translate_exp(root->child[0],op1);
        intercodes_merge(codes,code1);
        ...
        intercodes* code2=translate_args(root->child[2],arg_head);
...
intercodes* array_assignop(ASTNode* root){
    ...
    int dim1=0;
    int dim2=0;
    symbol* entry1=struct_array_type(root->child[0],&dim1);
    symbol* entry2=struct_array_type(root->child[2],&dim2);
    if(entry1->dim==dim1){
        ...
        symbol* entry3=struct_array_offset(root->child[0],&offset1,&array_head1,codes,&temp1);
        int offset2=0;
        int temp2=0;
        array_list* array_head2=NULL;
        symbol* entry4=struct_array_offset(root->child[2],&offset2,&array_head2,codes,&temp2);
